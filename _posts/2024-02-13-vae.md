...
\begin{equation} \text{Encoder computes } \label{eq:vae_encoder} q_\Phi(z|x) \end{equation}    
\begin{equation} \text{Decoder computes } \label{eq:vae_decoder} p_\theta(x|z) \end{equation}    
...
This way we learn z $$q_\Phi(z|x)$$ (\eqref{vae_encoder} Encoder) as close to the prior $$p(z)$$ 
...
Equation \eqref{eq:loss_vae_regularzn} is actually the KL divergence between the 
two distributions.   
