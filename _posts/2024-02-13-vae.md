...
$$ \frac{ \partial z_i}{\partial \sigma_i} = \epsilon_i $$. $$ \mu $$ and $$ \sigma $$ are learned
 so they can be backpropagated and $$ \epsilon $$ is sampled from a distribution.    
Because this distribution is Gaussian, we *know* an estimator and can calculate it relatively easier.
