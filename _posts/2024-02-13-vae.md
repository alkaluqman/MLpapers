...
$$ \underbrace{p_{\theta}(z|x)}\_{\text{TRUE posterior}} \approx \underbrace{q\_{\phi}(z|x)}\_{\text{APPROX posterior}} $$   
...
$$ \text{log }p_\theta(x)= \mathbb{E}\_{q\_{\phi}(z|x)} \[\text{log }p\_{\theta}(x)\] $$   
